...
st.set_page_config(page_title="통합 장비 관리 시스템", layout="wide")
...
def init_user_db():
    ...
    if not os.path.exists(USER_FILE_NAME):
        ...
        # ▼▼▼ [수정 포인트] 관리자 초기 아이디/비번 설정 ▼▼▼
        admin_id = "admin"   # 원하는 아이디로 변경 가능
        admin_pw = "1234"    # 원하는 비밀번호로 변경 가능
        # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
...
def log_transaction(kind, item_name, qty, target, date_val, return_val=''):
    new_log = {'시간': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), '종류': kind, '장비이름': item_name, '수량': qty, '대상(현장/업체)': target, '날짜': date_val, '반납예정일': return_val}
...
def main_app():
    ...
    with tabs[0]:
        ...
        with st.expander("➕ 새 장비 등록"):
            with st.form("add_form", clear_on_submit=True):
                ...
                new_type = c1.text_input("타입"); new_name = c2.text_input("이름"); new_count = c3.number_input("수량", min_value=1, value=1)
                c4, c5, c6 = st.columns(3)
                new_brand = c4.text_input("브랜드"); new_lender = c5.text_input("대여업체"); new_note = c6.text_input("특이사항")
                if st.form_submit_button("등록"):
                    if new_name:
                        new_row = {'ID': str(uuid.uuid4()), '타입': new_type, '이름': new_name, '수량': new_count, '브랜드': new_brand, '특이사항': new_note, '대여업체': new_lender, '대여여부': '재고', '대여자': '', '대여일': '', '반납예정일': '', '출고비고': ''}
        ...
        def highlight_rows(row):
            ...
            elif status == '대여 중': return ['background-color: #fff2cc'] * len(row)
        ...
        if not view_df.empty:
            ...
            if st.button("선택 장비 영구 삭제", type="primary"):
                ...
                save_data(st.session_state.df); st.success("삭제되었습니다."); st.rerun()
        ...
        if stock_df.empty: st.info("대여 가능한 재고가 없습니다.")
            ...
            if sel_idx is not None:
                ...
                with st.form("rent_form"):
                    target = st.text_input("빌리는 업체명"); c1, c2, c3 = st.columns(3)
                    qty = c1.number_input("수량", min_value=1, max_value=int(item['수량']), value=1)
                    d_out = c2.date_input("대여일", datetime.now()); d_ret = c3.date_input("반납예정일", value=None)
                    if st.form_submit_button("대여 실행"):
                        if not target: st.error("업체명을 입력하세요.")
                        else:
                            date_s = d_out.strftime("%Y-%m-%d"); ret_s = d_ret.strftime("%Y-%m-%d") if d_ret else ""
                            if qty < item['수량']:
                            ...
                            save_data(st.session_state.df); st.success("대여 처리 완료"); st.rerun()
        ...
        if stock_df.empty: st.info("출고 가능한 재고가 없습니다.")
            ...
            if sel_idx is not None:
                ...
                with st.form("dispatch_form"):
                    target = st.text_input("현장명"); c1, c2, c3 = st.columns(3)
                    qty = c1.number_input("수량", min_value=1, max_value=int(item['수량']), value=1)
                    d_out = c2.date_input("출고일", datetime.now()); d_ret = c3.date_input("반납예정일", value=None); note = st.text_input("출고 비고")
                    if st.form_submit_button("출고 실행"):
                        if not target: st.error("현장명을 입력하세요.")
                            ...
                            save_data(st.session_state.df); st.success("출고 처리 완료"); st.rerun()
            ...
            if sel_idx is not None:
                ...
                with st.form("in_form"):
                    ...
                    if st.form_submit_button("반납 실행"):
                        mask = ((st.session_state.df['이름'] == item['이름']) & (st.session_state.df['브랜드'] == item['브랜드']) & (st.session_state.df['대여업체'] == item['대여업체']) & (st.session_state.df['특이사항'] == item['특이사항']) & (st.session_state.df['대여여부'] == '재고'))
                        merge_idx = st.session_state.df[mask].index
                        if qty < item['수량']:
                            st.session_state.df.at[sel_idx, '수량'] -= qty
                            if not merge_idx.empty: st.session_state.df.at[merge_idx[0], '수량'] += qty
                            else:
                                new_row = item.copy(); new_row['ID'] = str(uuid.uuid4()); new_row['수량'] = qty; new_row['대여여부'] = '재고'; new_row['대여자'] = ''; new_row['대여일'] = ''; new_row['반납예정일'] = ''; new_row['출고비고'] = ''
                                st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([new_row])], ignore_index=True)
                        else:
                            if not merge_idx.empty: st.session_state.df.at[merge_idx[0], '수량'] += qty; st.session_state.df = st.session_state.df.drop(sel_idx).reset_index(drop=True)
                            else: st.session_state.df.at[sel_idx, '대여여부'] = '재고'; st.session_state.df.at[sel_idx, '대여자'] = ''; st.session_state.df.at[sel_idx, '대여일'] = ''; st.session_state.df.at[sel_idx, '반납예정일'] = ''; st.session_state.df.at[sel_idx, '출고비고'] = ''
                        log_transaction("반납", item['이름'], qty, item['대여자'], datetime.now().strftime("%Y-%m-%d"))
                        save_data(st.session_state.df); st.success("반납 완료"); st.rerun()
                        ...
                        if item['대여여부'] == target_st: st.warning("이미 해당 상태입니다.")
                            ...
                            if qty < item['수량']:
                                ...
                                if not merge_idx.empty: st.session_state.df.at[merge_idx[0], '수량'] += qty
                                    ...
                                    new_row = item.copy(); new_row['ID'] = str(uuid.uuid4()); new_row['수량'] = qty; new_row['대여여부'] = target_st; 
                                    new_row['대여자'] = target_st if target_st != '재고' else ''; new_row['대여일'] = ''; new_row['반납예정일'] = ''
                                    st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([new_row])], ignore_index=True)
                            else:
                                if not merge_idx.empty: st.session_state.df.at[merge_idx[0], '수량'] += qty; st.session_state.df = st.session_state.df.drop(sel_idx).reset_index(drop=True)
                                else: st.session_state.df.at[sel_idx, '대여여부'] = target_st; st.session_state.df.at[sel_idx, '대여자'] = target_st if target_st != '재고' else ''; st.session_state.df.at[sel_idx, '대여일'] = ''; st.session_state.df.at[sel_idx, '반납예정일'] = ''
                            log_transaction(f"상태변경({target_st})", item['이름'], qty, target_st, datetime.now().strftime("%Y-%m-%d"))
                            save_data(st.session_state.df); st.success(f"{target_st} 변경 완료"); st.rerun()
    ...
    with tabs[5]:
        ...
        if os.path.exists(LOG_FILE_NAME):
            ...
            st.download_button("CSV 다운로드", csv_d, "logs.csv", "text/csv")
        ...
        with tabs[6]:
            ...
            if pending_users.empty: st.info("대기 중인 회원이 없습니다.")
                ...
                for idx, row in pending_users.iterrows():
                    ...
                    c1.write(f"**{row['username']}**"); c2.write(f"{row['created_at']}")
                    if c3.button("승인", key=f"app_{idx}"): update_user_status(row['username'], "approve"); st.success("승인 완료"); st.rerun()
                    if c4.button("거절", key=f"rej_{idx}"): update_user_status(row['username'], "delete"); st.warning("삭제 완료"); st.rerun()
            ...
            for idx, row in approved_users.iterrows():
                ...
                if c3.button("추방", key=f"del_{idx}"): update_user_status(row['username'], "delete"); st.rerun()
